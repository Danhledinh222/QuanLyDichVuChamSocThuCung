...
namespace PetcareWebsite.Controllers
{
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var roleId = HttpContext.Session.GetRoleId();
            ...
            var weeklyBookingDates = await _context.Bookings
                .Where(b => b.IsDeleted != true && b.BookingDate >= weekStart && b.BookingDate < tomorrow)
                .Select(b => b.BookingDate)
                .ToListAsync();

            var weeklyPayments = await _context.Payments
                .Where(p => p.PaymentDate != null && p.PaymentDate >= weekStart && p.PaymentDate < tomorrow)
                .Select(p => new { PaymentDate = p.PaymentDate!.Value, p.Amount })
                .ToListAsync();
            ...
            var recentBookings = await _context.Bookings
                ...
                .OrderByDescending(b => b.CreatedAt ?? b.BookingDate)
                .Take(6)
                .ToListAsync();
            ...
            var model = new AdminDashboardViewModel
            {
                ...
                TodayBookingCount = await _context.Bookings.CountAsync(b =>
                    b.IsDeleted != true &&
                    b.BookingDate >= today &&
                    b.BookingDate < tomorrow),
                PendingBookingCount = await _context.Bookings.CountAsync(b =>
                    b.IsDeleted != true &&
                    (b.StatusId == BookingStatusPending ||
                     b.StatusId == BookingStatusConfirmed ||
                     b.StatusId == BookingStatusInProgress)),
                CustomerCount = await _context.Customers.CountAsync(c => c.IsDeleted != true),
                PetCount = await _context.Pets.CountAsync(p => p.IsDeleted != true),
                MonthlyRevenue = await _context.Payments
                    .Where(p => p.PaymentDate != null && p.PaymentDate >= monthStart && p.PaymentDate < tomorrow)
                    .SumAsync(p => (decimal?)p.Amount) ?? 0m,
                NewContactCount = await _context.ContactMessages.CountAsync(m => m.Status == "New"),
                LowStockCount = await _context.MedicalSupplies.CountAsync(s =>
                    s.IsDeleted != true &&
                    (s.StockQuantity ?? 0) <= (s.MinStockLevel ?? 0)),
                RecentContacts = await _context.ContactMessages
                    .OrderByDescending(m => m.CreatedAt)
                    .Take(5)
                    .ToListAsync(),
                LowStockSupplies = await _context.MedicalSupplies
                    .Where(s => s.IsDeleted != true && (s.StockQuantity ?? 0) <= (s.MinStockLevel ?? 0))
            ...
            for (var i = 0; i < 7; i++)
            {
                ...
                model.WeeklyBookingCounts.Add(weeklyBookingDates.Count(d => d.Date == date));
        ...
        public async Task<IActionResult> Bookings(string? search, int? statusId)
        {
            ...
            var model = new AdminBookingsViewModel
            {
                ...
                PendingCount = await allBookings.CountAsync(b => b.StatusId == BookingStatusPending),
                InProgressCount = await allBookings.CountAsync(b => b.StatusId == BookingStatusInProgress),
                CompletedCount = await allBookings.CountAsync(b => b.StatusId == BookingStatusCompleted),
                CancelledCount = await allBookings.CountAsync(b => b.StatusId == BookingStatusCancelled),
                ExpiredCount = await allBookings.CountAsync(b => b.StatusId == BookingStatusExpired),
        ...
        public async Task<IActionResult> Promotions(string? search, string? status)
        {
            ...
            var model = new AdminPromotionsViewModel
            {
                Search = search,
                Status = status,
        ...
        public IActionResult CreatePromotion()
        {
            ...
            var model = new AdminPromotionEditorViewModel
            {
        ...
        public async Task<IActionResult> SaveBooking(AdminBookingEditorViewModel model)
        {
            ...
            {
                var customer = await _context.Customers
                    .FirstOrDefaultAsync(c => c.CustomerId == model.CustomerId && c.IsDeleted != true);
                var pet = await _context.Pets
                    .FirstOrDefaultAsync(p => p.PetId == model.PetId
            ...
            var service = await _context.ServiceCatalogs
                .FirstOrDefaultAsync(s => s.ServiceId == model.ServiceId
                    && ((s.IsActive == true && s.IsDeleted != true) ||
                        (model.BookingId.HasValue &&
                         s.BookingDetails.Any(detail => detail.BookingId == model.BookingId.Value))));
            ...
            if (model.BookingId.HasValue)
            {
                return await UpdateBookingAsync(model, service!);
            }

            return await AddBookingAsync(model, service!, isGuestBooking);
        }
            ...
            bool isGuestBooking)
        {
            ...
            var statusId = allowedStatuses.Contains(model.StatusId) ? model.StatusId : BookingStatusConfirmed;
            var now = DateTime.Now;
        ...
        private async Task<IActionResult> UpdateBookingAsync(AdminBookingEditorViewModel model, ServiceCatalog service)
        {
            var booking = await _context.Bookings
                ...
                .Include(b => b.BookingDetails)
                    .ThenInclude(detail => detail.BookingDetailEmployees)
                .FirstOrDefaultAsync(b => b.BookingId == model.BookingId && b.IsDeleted != true);
